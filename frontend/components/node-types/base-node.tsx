...
export const BaseNode = ({
  ...
  const handleStyle =
    "!bg-primary w-3 h-3 transition-transform hover:scale-150";
      ...
      {singleTarget ? (
        ...
        <>
          <Handle
            ...
            className={`${handleStyle} left-[25%]`}
          />
          <Handle
            type="target"
            position={Position.Top}
            className={`${handleStyle} left-[75%]`}
          />
        </>
      )}
      ...
      <CardContent className="p-4 pt-2">
        ...
        {data.file && (
          <div className="mt-2 p-2 bg-muted rounded-md">
            <p className="text-xs font-mono truncate">📎 {data.file.name}</p>
          </div>
        )}
      </CardContent>
    </Card>
